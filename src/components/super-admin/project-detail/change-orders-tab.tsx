  ...
  return (
    <div>
      <div className="grid mb-7" style={{ gridTemplateColumns: "1fr 1fr 1fr", borderTop: "1px solid var(--kas-ink)", borderBottom: "1px solid var(--kas-ink)" }}>
        {[{ l: "Total Change Order", v: String(orders.length).padStart(2, "0"), accent: false }, { l: "Nilai Disetujui", v: fmtIDR(approved), accent: false }, { l: "Menunggu Persetujuan", v: fmtIDR(pending), accent: pending > 0 }].map((s, i) => (
        ...
        {orders.map((co, i) => {
          ...
          return (
            <div key={co.id} className="grid gap-5 items-start py-5" style={{ gridTemplateColumns: "40px 1fr auto auto auto", borderBottom: "1px solid var(--kas-line)" }}>
              ...
              <div style={{ textAlign: "right", fontFamily: "var(--font-jetbrains), monospace", fontSize: 14, color: status === "Ditolak" ? "var(--kas-ink-3)" : co.costImpact > 0 ? "var(--kas-rust)" : "var(--kas-ink)", textDecoration: status === "Ditolak" ? "line-through" : "none" }}>+{fmtIDR(co.costImpact)}</div>
              ...
              <div className="flex gap-1.5">
                {status === "Menunggu" && (
                  <>
                    <button type="button" onClick={() => setCoStatuses((p) => ({ ...p, [co.id]: "Disetujui" }))} style={{ background: "var(--kas-ink)", color: "var(--kas-paper)", border: "none", padding: "6px 10px", fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", cursor: "pointer" }}>Setujui</button>
                    <button type="button" onClick={() => setCoStatuses((p) => ({ ...p, [co.id]: "Ditolak" }))} style={{ background: "transparent", border: "1px solid var(--kas-line)", padding: "6px 10px", fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", cursor: "pointer", color: "var(--kas-ink-3)" }}>Tolak</button>
                  </>
                )}
              </div>
            </div>
