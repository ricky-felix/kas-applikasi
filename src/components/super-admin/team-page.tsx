...
export default function TeamPage() {
	const workerData = WORKERS.map((w) => {
		...
		const revCredit = payroll
			? payroll.projects.reduce((s, proj) => {
					...
					return (
						s +
						(projAllDays > 0 ? project.paid * (workerDays / projAllDays) : 0)
					);
				}, 0)
			: 0;
	...
	const activeWorkers = workerData.filter(
		(w) => w.assignedProjects.length > 0,
	).length;
	...
	return (
		<div
			className="px-9 py-7 pb-14"
			style={{
				fontFamily: "var(--font-manrope), sans-serif",
				color: "var(--kas-ink)",
			}}
		>
			<TopBar title="Pekerja Lapangan" />
				...
				{[
					{
						...
						accent: "var(--kas-cobalt)",
					},
					{
						label: "Total hari kerja Mei",
						value: String(totalDaysAll),
						sub: "semua pekerja gabungan",
						accent: "var(--kas-moss)",
					},
					{
						label: "Laporan terkirim",
						value: String(totalReports),
						sub: "work reports bulan ini",
						accent: "var(--kas-ochre)",
					},
				].map((s, i) => (
					<div
						key={i}
							...
							<div className="flex items-start gap-2.5">
								...
								<div>
									<div className="flex items-center gap-2">
										...
										{w.isKepalaProyek && (
											<span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 8, letterSpacing: "0.12em", textTransform: "uppercase", background: "var(--kas-ink)", color: "var(--kas-paper)", padding: "1px 5px", flexShrink: 0 }}>TL</span>
										)}
									</div>
									<div
										style={{
							...
							<div className="flex flex-col gap-2.5">
								{w.assignedProjects.length === 0 ? (
									...
									w.assignedProjects.map((p) => {
										const dp = w.daysPerProject.find(
											(d) => d.projectId === p.id,
										);
										return (
											<div key={p.id}>
							...
							<div style={{ textAlign: "right" }}>
								{w.revPerDay > 0 ? (
									<>
										...
										<div className="flex justify-end mt-2">
											<MiniBar
												pct={(w.revPerDay / maxRevPerDay) * 100}
												color="var(--kas-ink)"
											/>
										</div>
										<div
									...
									<>
										...
										<select
											...
											style={{
												...
												border: `1px solid ${wageStatus[w.id] === "lunas" ? "var(--kas-line)" : "var(--kas-ink)"}`,
												background: wageStatus[w.id] === "lunas" ? "var(--kas-moss-soft)" : "var(--kas-paper)",
												color: wageStatus[w.id] === "lunas" ? "var(--kas-moss-ink)" : "var(--kas-ink)",
												cursor: "pointer",
												appearance: "none",
