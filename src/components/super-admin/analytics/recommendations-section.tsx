...
import type { AnalyticsData, Recommendation, RecType, PendingItem } from "./types";
...
function buildRecs(d: AnalyticsData): Recommendation[] {
  ...
  d.lowStockMaterials.forEach(m => {
    recs.push({ type: "danger", label: "Stok Material", nav: "Material",
      text: `${m.name} di bawah minimum — ${m.stock} ${m.unit} tersisa (min. ${m.minStock} ${m.unit}). Hubungi ${m.supplier}.`,
    });
  });

  const pendingMR = d.pendingItems.filter((p): p is Extract<PendingItem, { kind: "material" }>     => p.kind === "material");
  const pendingCO = d.pendingItems.filter((p): p is Extract<PendingItem, { kind: "change_order" }> => p.kind === "change_order");

  if (pendingMR.length > 0) {
    recs.push({ type: "warning", label: "Permintaan Material", nav: "Material",
  ...
  if (pendingCO.length > 0) {
    ...
    recs.push({ type: "warning", label: "Change Order", nav: "Proyek",
  ...
  const inquiryConvPct = d.website.funnel.visitors > 0 ? (d.website.funnel.inquiries / d.website.funnel.visitors) * 100 : 0;
  if (inquiryConvPct < 5) recs.push({ type: "info", label: "Situs Web", text: `Konversi pengunjung ke inquiry hanya ${inquiryConvPct.toFixed(1)}% — tambahkan tombol CTA di halaman Portofolio dan Beranda.` });
  if (cur.bounceRate > 50) recs.push({ type: "info", label: "Situs Web", text: `Bounce rate ${cur.bounceRate}% — halaman Kontak paling cepat ditinggalkan. Sederhanakan formulir kontak.` });

  const lowUtilWorkers = d.employees.filter(w => w.utilPct < 55);
  if (lowUtilWorkers.length > 0) {
    recs.push({ type: "info", label: "Tim", nav: "Tim & Absensi",
    ...
    <section>
      ...
      <div style={{ borderTop: "1px solid var(--kas-ink)" }}>
        {recs.length === 0 ? (
          ...
          return (
            <div key={i} className="grid gap-4 items-start py-4" style={{ gridTemplateColumns: "8px 100px 1fr auto", borderBottom: "1px solid var(--kas-line)", background: c.bg }}>
              <div style={{ width: 8, height: 8, background: c.dot, marginTop: 4, flexShrink: 0 }} />
              <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.14em", color: c.dot, textTransform: "uppercase", fontWeight: 700, paddingTop: 3 }}>{r.label}</span>
              <span style={{ fontFamily: "var(--font-manrope), sans-serif", fontSize: 13, lineHeight: 1.5 }}>{r.text}</span>
              {r.nav && <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.12em", color: "var(--kas-ink-3)", textTransform: "uppercase", whiteSpace: "nowrap", paddingTop: 3 }}>→ {r.nav}</span>}
            </div>
          );
        })}
      </div>
