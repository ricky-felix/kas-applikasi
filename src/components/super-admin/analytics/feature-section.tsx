...
export function FeatureSection({ d }: { d: AnalyticsData }) {
  return (
    <section className="mb-12">
      ...
      <SecTitle>Penggunaan fitur, <em>bulan ini.</em></SecTitle>
      ...
      <div className="grid gap-9" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <div style={{ borderTop: "1px solid var(--kas-ink)" }}>
          {d.features.map((f, i) => (
            <div key={i} className="py-3.5" style={{ borderBottom: "1px solid var(--kas-line)" }}>
              ...
              <HBar pct={(f.value / f.cap) * 100} h={4} color="var(--kas-cobalt)" />
        ...
        <div>
          <div style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.16em", color: "var(--kas-ink-3)", textTransform: "uppercase", marginBottom: 10 }}>Antrian persetujuan</div>
          <div style={{ borderTop: "1px solid var(--kas-ink)" }}>
            {d.pendingItems.length === 0 ? (
              <div className="py-5" style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 10, color: "var(--kas-ink-3)", letterSpacing: "0.1em" }}>Tidak ada item menunggu.</div>
            ) : d.pendingItems.map((item: PendingItem, i) =>
              item.kind === "material" ? (
                <div key={i} className="py-3" style={{ borderBottom: "1px solid var(--kas-line)" }}>
                  <div className="flex justify-between items-baseline">
                    <span style={{ fontFamily: "var(--font-manrope), sans-serif", fontSize: 13 }}>{item.materialName}</span>
                ...
                <div key={i} className="py-3" style={{ borderBottom: "1px solid var(--kas-line)" }}>
                  <div className="flex justify-between items-baseline">
                    <span style={{ fontFamily: "var(--font-manrope), sans-serif", fontSize: 13, flex: 1, marginRight: 8 }}>{item.description.length > 40 ? item.description.slice(0, 40) + "…" : item.description}</span>
                  ...
                  <div style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, color: "var(--kas-ink-3)", marginTop: 3, letterSpacing: "0.08em" }}>
                    {fmtIDRshort(item.costImpact)} · {item.requestedBy} · {item.date}
                  </div>
                </div>
              )
            )}
