import {
  PROJECTS, PAYROLL_MAY, MATERIALS, WORKERS,
...
export function getMockData(): AnalyticsData {
  const employees = PAYROLL_MAY.map(pw => {
    ...
    return { workerId: pw.workerId, name: pw.name, workerRole: worker?.role ?? "", rate: pw.rate, totalDays, halfDays, wages, utilPct, reports, requests, revPerDay };
  ...
  return {
    website: { monthly: WEBSITE_MONTHLY, pages: WEBSITE_PAGES, funnel: WEBSITE_FUNNEL },
    posthog: {
      totalSessions: 47, totalEvents: 312, activeUsers: 5,
      byRole: [
        { role: "worker",      label: "Pekerja",    users: 2, sessions: 31, events: 248, topEvent: "Absen masuk"      },
        { role: "admin",       label: "Admin",       users: 1, sessions: 9,  events: 44,  topEvent: "Setujui material" },
        { role: "owner",       label: "Pemilik",     users: 1, sessions: 5,  events: 14,  topEvent: "Buka dashboard"   },
        { role: "super_admin", label: "Super Admin", users: 1, sessions: 2,  events: 6,   topEvent: "Buka analitik"    },
      ],
      topEvents: [
        ...
        { event: "worker_report_submitted", label: "Laporan dikirim",    count: 18 },
        { event: "admin_material_approved", label: "Material disetujui", count: 11 },
      ],
    },
    employees,
    features: [
      { label: "Laporan kerja dikirim", value: WORK_REPORTS.length,      cap: 20 },
      { label: "Permintaan material",   value: MATERIAL_REQUESTS.length, cap: 10 },
      { label: "Change order",          value: CHANGE_ORDERS.length,     cap: 6  },
    ],
    pendingItems: [
      ...pendingMR.map(r  => ({ kind: "material"     as const, id: r.id,  materialName: r.materialName, workerName: r.workerName, qty: r.qty, unit: r.unit, date: r.date })),
      ...pendingCO.map(co => ({ kind: "change_order" as const, id: co.id, description: co.description, costImpact: co.costImpact, requestedBy: co.requestedBy, date: co.date })),
    ],
    lowStockMaterials: MATERIALS.filter(m => m.stock <= m.minStock).map(m => ({ name: m.name, stock: m.stock, minStock: m.minStock, unit: m.unit, supplier: m.supplier })),
  };
}
