...
export default function AMBilling({ toast }: { toast: (m: string) => void }) {
  ...
  // Demo: first project has a proof already submitted by the client
  const [proofsReceived] = useState<Set<string>>(new Set([outstanding[0]?.id]));
  ...
  return (
    <div className="px-5 pt-4 pb-6">
      ...
      <div className="mt-4 p-4" style={{ background: "var(--kas-ink)", color: "var(--kas-paper)" }}>
        ...
        <div style={{ fontFamily: "var(--font-newsreader), serif", fontSize: 36, fontWeight: 500, marginTop: 6, color: "var(--kas-rust)" }}>{fmtIDR(total)}</div>
      </div>
        ...
        {outstanding.map((p) => {
          ...
          return (
            <div key={p.id} className="py-4" style={{ borderBottom: "1px solid var(--kas-line)" }}>
              ...
              {proofsReceived.has(p.id) && !isLunas && (
                <div className="mt-2.5 flex items-center gap-2 px-3.5 py-2" style={{ border: "1px solid var(--kas-ochre)", background: "var(--kas-ochre-soft)" }}>
                  <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--kas-ochre-ink)" }}>📎 Bukti transfer diterima · perlu verifikasi</span>
                </div>
              )}
              {isLunas ? (
                <div className="mt-2.5 flex items-center gap-2 px-3.5 py-2.5" style={{ border: "1px solid var(--kas-line)", background: "var(--kas-moss-soft)" }}>
                ...
                <div className="grid gap-1.5 mt-2.5" style={{ gridTemplateColumns: "1fr 1fr" }}>
                  ...
                  <a
                    ...
                    style={{ border: "none", background: "var(--kas-ink)", color: "var(--kas-paper)", padding: "10px 0", fontFamily: "var(--font-jetbrains), monospace", fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", textDecoration: "none", display: "flex" }}
                  >
                    <WAIcon />
                    Tagih WA
                  </a>
