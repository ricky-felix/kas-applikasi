...
export default function AMLaporan() {
  return (
    <div className="px-5 pt-4 pb-6">
      <Kicker no="02" label={`${WORK_REPORTS.length} LAPORAN HARI INI`} />
        ...
        {WORK_REPORTS.map((r) => (
          <div key={r.id} className="py-4" style={{ borderBottom: "1px solid var(--kas-line)" }}>
            <div className="flex items-center gap-2.5 mb-2">
              <div className="grid place-items-center" style={{ width: 32, height: 32, background: "var(--kas-paper-2)", border: "1px solid var(--kas-line)", fontFamily: "var(--font-newsreader), serif", fontSize: 13, fontWeight: 500, flexShrink: 0 }}>
                {r.workerShort}
              </div>
              <div className="flex-1">
                ...
                <div style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, color: "var(--kas-ink-3)", marginTop: 1, letterSpacing: "0.08em" }}>
                  {PROJECTS.find((p) => p.id === r.projectId)?.code}
                </div>
              </div>
            </div>
            <p style={{ fontFamily: "var(--font-newsreader), serif", fontSize: 15, lineHeight: 1.5, color: "var(--kas-ink-2)", margin: 0 }}>{r.note}</p>
            {r.photos > 0 && (
              <div style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, color: "var(--kas-cobalt)", marginTop: 5, letterSpacing: "0.12em", textTransform: "uppercase" }}>
                {r.photos} foto terlampir
              </div>
            )}
          </div>
        ))}
