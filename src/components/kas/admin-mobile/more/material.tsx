...
export default function AMMaterial({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="px-5 pt-4 pb-6">
      ...
      <Kicker no="05" label={`${MATERIALS.length} ITEM MATERIAL`} />
      <div className="flex justify-between items-end mb-4">
        <DisplayHeading size={26}>Material,<br /><em>gudang.</em></DisplayHeading>
        {lowCount > 0 && (
          <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", padding: "4px 10px", background: "var(--kas-rust-soft)", color: "var(--kas-rust-ink)" }}>
            {lowCount} stok tipis
          </span>
        )}
      </div>
