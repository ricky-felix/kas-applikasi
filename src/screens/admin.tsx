...
type FeedItem = {
	type: "hadir" | "laporan" | "material" | "daftar";
	who: string;
	action: string;
	t: string;
	meta?: string;
...
function buildFeed(
	...
	WORK_REPORTS: ReturnType<typeof useWorkReports>,
	MATERIAL_REQUESTS: ReturnType<typeof useMaterialRequests>,
	PENDING_REGISTRATIONS: ReturnType<typeof usePendingRegistrations>,
): FeedItem[] {
	return [
		...PROJECTS.flatMap((p) =>
			p.activity.map((a) => ({
...
function NotificationPanel({ FEED, onClose }: { FEED: FeedItem[]; onClose: () => void }) {
	return (
		<div
			className="fixed inset-0 z-40 flex flex-col"
			style={{ background: "rgba(22,28,44,0.45)" }}
			onClick={onClose}
		>
			<div
				...
				style={{
					...
					borderBottom: "2px solid var(--kas-ink)",
				}}
...
export default function AdminMobile({
	...
	const FEED = useMemo(
		() => buildFeed(PROJECTS, WORK_REPORTS, MATERIAL_REQUESTS, PENDING_REGISTRATIONS),
		[PROJECTS, WORK_REPORTS, MATERIAL_REQUESTS, PENDING_REGISTRATIONS],
	);
	...
	return (
		...
		>
			<MobileTopBar tabLabel={tabLabels[tab]}>
				<button
					...
					style={{
						...
						background: showNotif ? "var(--kas-ink)" : "transparent",
						color: showNotif ? "var(--kas-paper)" : "var(--kas-ink-2)",
						width: 32,
						height: 32,
						cursor: "pointer",
						flexShrink: 0,
			...
			{showNotif && <NotificationPanel FEED={FEED} onClose={() => setShowNotif(false)} />}

			<div className="flex-1 overflow-y-auto">
				{tab === "home" && <AMHome />}
				{tab === "projects" && projSub === "status" && (
					<AMProjects toast={toast} />
				)}
				{tab === "projects" && projSub === "laporan" && <AMLaporan />}
				{tab === "requests" && <AMRequests toast={toast} />}
				{tab === "billing" && <AMFinancial toast={toast} />}
				{tab === "more" && (
					<AMMore session={sess} toast={toast} onLogout={onLogout} />
				)}
			</div>
